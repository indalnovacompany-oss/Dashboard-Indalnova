...
export async function generateInvoice(orders) {
  for (const order of orders) {
    await generateSingleInvoice(order); // wait for each PDF
  }
}

function generateSingleInvoice(order) {
  return new Promise((resolve, reject) => {
    ...
    doc.font("Helvetica").fontSize(10).text("123H PATEL NAGAR, RAMADEVI, KANPUR, UTTAR PRADESH - 208007", 50, 65);
    doc.text(`Email: teamindalnova@gmail.com | Phone: +91-8840393051`, 50, 80);

    // ===== Invoice Info Box =====
    doc.rect(400, 50, 150, 50).stroke();
    doc.font("Helvetica-Bold").fontSize(14).text("INVOICE", 410, 55);
    doc.font("Helvetica").fontSize(9).text(`Invoice Date: ${new Date().toLocaleDateString()}`, 410, 75);
    doc.text(`Order ID: ${order.order_id}`, 410, 90);
